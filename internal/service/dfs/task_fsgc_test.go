...
func TestFSGC(t *testing.T) {
	...
	t.Run("RunArgs Success", func(t *testing.T) {
		...
		inodesMock.On("GetAllInodesWithFileID", mock.Anything, *inodes.ExampleAliceFile.FileID()).Return([]inodes.INode{}, nil).Once()
		...
		inodesMock.On("HardDelete", mock.Anything, &inodes.ExampleAliceRoot).Return(nil).Once()
