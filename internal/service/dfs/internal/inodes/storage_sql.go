...
var allFiels = []string{"id", "name", "parent", "size", "space_id", "last_modified_at", "created_at", "file_id"}
...
func (s *sqlStorage) GetSpaceRoot(ctx context.Context, spaceID uuid.UUID) (*INode, error) {
	return s.getByKeys(ctx, sq.Eq{"space_id": string(spaceID), "parent": nil})
}
...
func (s *sqlStorage) Save(ctx context.Context, i *INode) error {
	_, err := sq.
		Insert(tableName).
		Columns(allFiels...).
		Values(i.id, i.name, i.parent, i.size, i.spaceID, i.lastModifiedAt, i.createdAt, i.fileID).
		RunWith(s.db).
		ExecContext(ctx)
	if err != nil {
...
func (s *sqlStorage) scanRows(rows *sql.Rows) ([]INode, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&res.id, &res.name, &res.parent, &res.size, &res.spaceID, &res.lastModifiedAt, &res.createdAt, &res.fileID)
		if err != nil {
...
func (s *sqlStorage) GetByNameAndParent(ctx context.Context, name string, parent uuid.UUID) (*INode, error) {
	...
	err := sq.
		...
		RunWith(s.db).
		ScanContext(ctx, &res.id, &res.name, &res.parent, &res.size, &res.spaceID, &res.lastModifiedAt, &res.createdAt, &res.fileID)
