...
var allFields = []string{"id", "username", "admin", "status", "password", "created_at"}
...
func (s *sqlStorage) Save(ctx context.Context, user *User) error {
	_, err := sq.
		Insert(tableName).
		Columns(allFields...).
		Values(user.id, user.username, user.isAdmin, user.status, user.password, user.createdAt).
		RunWith(s.db).
		ExecContext(ctx)
	if err != nil {
...
func (s *sqlStorage) getByKeys(ctx context.Context, wheres ...any) (*User, error) {
	...
	err := query.
		RunWith(s.db).
		ScanContext(ctx, &res.id, &res.username, &res.isAdmin, &res.status, &res.password, &res.createdAt)
...
func (s *sqlStorage) scanRows(rows *sql.Rows) ([]User, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&res.id, &res.username, &res.isAdmin, &res.status, &res.password, &res.createdAt)
		if err != nil {
