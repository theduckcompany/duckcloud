...
func (s *davFS) Mkdir(ctx context.Context, name string, perm os.FileMode) error {
	...
	return fs.NewFSService(currentUser, root, s.inodes, s.files, s.tools).CreateDir(ctx, name, perm)
}

func (s *davFS) OpenFile(ctx context.Context, name string, flag int, perm os.FileMode) (webdav.File, error) {
	return fs.NewFSService(currentUser, root, s.inodes, s.files, s.tools).OpenFile(ctx, name, flag, perm)
}

func (s *davFS) RemoveAll(ctx context.Context, name string) error {
	return fs.NewFSService(currentUser, root, s.inodes, s.files, s.tools).RemoveAll(ctx, name)
}
...
func (s *davFS) Stat(ctx context.Context, name string) (os.FileInfo, error) {
	return fs.NewFSService(currentUser, root, s.inodes, s.files, s.tools).Stat(ctx, name)
}
