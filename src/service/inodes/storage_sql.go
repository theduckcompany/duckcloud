...
func (s *sqlStorage) Save(ctx context.Context, dir *INode) error {
	_, err := sq.
		Insert(tableName).
		Columns("id", "user_id", "name", "parent", "mode", "block_id", "last_modified_at", "created_at").
		Values(dir.id, dir.userID, dir.name, dir.parent, dir.mode, dir.blockID, dir.lastModifiedAt, dir.createdAt).
		RunWith(s.db).
		ExecContext(ctx)
	if err != nil {
...
func (s *sqlStorage) GetByID(ctx context.Context, id uuid.UUID) (*INode, error) {
	...
	err := sq.
		Select("id", "user_id", "name", "parent", "mode", "block_id", "last_modified_at", "created_at").
		From(tableName).
		Where(sq.Eq{"id": string(id), "deleted_at": nil}).
		RunWith(s.db).
		ScanContext(ctx, &res.id, &res.userID, &res.name, &res.parent, &res.mode, &res.blockID, &res.lastModifiedAt, &res.createdAt)
...
func (s *sqlStorage) scanRows(rows *sql.Rows) ([]INode, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&res.id, &res.userID, &res.name, &res.parent, &res.mode, &res.blockID, &res.lastModifiedAt, &res.createdAt)
		if err != nil {
